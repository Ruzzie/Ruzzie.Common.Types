...
public class ResultTests
{
    ...
    public void Cannot_InferTypesWhenTypesAreTheSame_Ok()
    {
        ...
        sutCtorOk.Should().Be(sutStaticOk);
    }
    ...
    public void Cannot_InferTypesWhenTypesAreTheSame_Err()
    {
        ...
        sutCtorErr.Should().Be(sutStaticErr);
    }
    ...
    {
        var result = new Result<string, int>(42);
        var actual = result.Match(s => s, i => i.ToString());
    ...
    {
        var result = new Result<string, int>("foo");
        var actual = result.Match(s => s, i => i.ToString());
    ...
    public void ForOk()
    {
        ...
        sut.For(Assert.Fail, i => Assert.AreEqual(42, i));
    }
    ...
    {
        Result<uint, uint> sq(uint x)
        {
            return Result.Ok<uint, uint>(x * x);
        }

        Result<uint, uint> err(uint x)
        {
            return Result.Err<uint, uint>(x);
        }
    ...
    {
        static Result<uint, uint> sq(uint x)
        {
            return Result.Ok<uint, uint>(x * x);
        }

        static Result<uint, uint> err(uint x)
        {
            return Result.Err<uint, uint>(x);
        }
    ...
    {
        Result<uint, uint> sq(in uint x)
        {
            return Result.Ok<uint, uint>(x * x);
        }

        Result<uint, uint> err(in uint x)
        {
            return Result.Err<uint, uint>(x);
        }
    ...
    {
        static Result<uint, uint> sq(in uint x)
        {
            return Result.Ok<uint, uint>(x * x);
        }

        static Result<uint, uint> err(in uint x)
        {
            return Result.Err<uint, uint>(x);
        }
    ...
    [Theory]
    [TestCaseSource(nameof(BifunctorLawsData))]
    public void MapErrObeysFirstFunctorLaw(Result<string, int> e)
    {
        e.Should().Be(e.MapErr(Identity));
    }
    ...
    {
        e.Should().Be(e.Map(Identity, Identity));
    }
